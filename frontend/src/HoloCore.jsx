...
export default function HoloCore({ estado, wakeFlash, onClick }) {
  ...
  useEffect(() => {
    ...
    const matAnillo1 = new THREE.MeshBasicMaterial({ color: 0xF2A93B, wireframe: true, transparent: true, opacity: 0.8 });
    const matAnillo2 = new THREE.MeshBasicMaterial({ color: 0xF2A93B, wireframe: true, transparent: true, opacity: 0.5 });
    ...
    const partMat = new THREE.PointsMaterial({ color: 0xF2A93B, size: 0.03, transparent: true, opacity: 0.75 });
